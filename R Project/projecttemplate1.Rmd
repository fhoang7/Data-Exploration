...
## Correlations Takeaways
In order to figure out which chemical qualities to explore, I performed a correlation test to evaluate if certain chemical qualities had correlations with quality. Using the results of the correlation test, I picked chemicalproperties that had at least a .25 Pearson's r value (can be negative for negative correlation as well).

```{r echo = FALSE, message=FALSE, warning=FALSE, Create_Bi_Plots}
...
## How do chemical properties vary based on quality score?

```{r echo = FALSE, message=FALSE, warning=FALSE, AlcoholQuality}
quality_boxplot(wine$alcohol) + ylab('Alcohol Content') #13
```

### Alcohol vs Quality
Can observe that there is a positive trend in alcohol content as quality scores go up. I would attribute the deviation in trend due to a lack of data points. 

```{r echo = FALSE, message=FALSE, warning=FALSE, SulphateQuality}
quality_boxplot(log10(wine$sulphates)) + ylab('Sulphate Content')
```

### Sulphate vs Quality
You can clearly see that as the quality score jumps so does the sulphate content. This is not surprising as sulphates are used as preservatives in wine. More preservatives probably makes the wine hold its flavor better.

```{r echo = FALSE, message=FALSE, warning=FALSE, CitricQuality}
quality_boxplot(wine$citric.acid)+ylab('Citric Acid') #15
```

### Citric Acid vs Quality
Wines with higher citric acid tend to have better quality. However, we can see that there are outliers in each rating that defies the trend in some cases. 

```{r echo = FALSE, message=FALSE, warning=FALSE, VolatileQuality}
quality_boxplot(wine$volatile.acidity)+ylab('Volatile Acidity')  #16
```

### Volatile Acid vs Quality
Based on this graph, it seems that wines with less volatile acidity perform better on quality scoring. However, once you get to higher scores, it seems that there is a law of diminishing returns in play here. Decreasing the volatile acidity when you get to the high quality wines does not seem to do much for the wine quality.  

## What makes a wine good, bad, or just ok?

```{r echo = FALSE, message=FALSE, warning=FALSE, AlcoholRating}
rating_boxplot(wine$alcohol)+ylab('Alcohol Content') #13
```

### Alcohol vs Rating
When we group wine alcohol content into the three rating groups, we see that higher alcohol wines have better perceived quality. I would question if there is that significant advantage between ok and good wines when you increase alcohol content that much due to the lack of a lot of good wine data. 

```{r echo = FALSE, message=FALSE, warning=FALSE, SulphateRating}
rating_boxplot(wine$sulphates) + ylab('Sulphate Content') #14
```

### Sulphate vs Rating
Wines with more sulphates tend to score better. Again, I would need to confirm this trend with a larger dataset as you can see that there are several ok wines that have very high sulphate content. 

```{r echo = FALSE, message=FALSE, warning=FALSE, CitricRating}
rating_boxplot(wine$citric.acid) + ylab('Citric Acid') #15
```

### Citric Acid vs Rating
I would not consider citric acid to be a predictor of quality since the distribution of data points for all wines are quite similar.  We would need to get more bad and good wine data to further investigate this potential correlation. 

```{r echo = FALSE, message=FALSE, warning=FALSE, }
rating_boxplot(wine$volatile.acidity)+ylab('Volatile Acidity')  #16
```

### Volatile Acid vs Rating
Good wines typically have the lowest volatile acidity. Again we observe what we saw when we split up the wine volatile acidity data by quality score. The difference in volatile acidity makes a huge difference between bad and ok wines in comparison to ok and good wines. 

# Bivariate Analysis
Having identified those to be alcohol sulphates, volatile acidity, and citric acid, I plotted those using boxplots to visualize how much of an influence a certain chemical property had on quality. I also slimmed down the quality scores into three ratings so I can get a sense for which properties generally determine whether a wine is good or bad.  
...
### Description
This bar graph quickly explains the wine dataset we are working with. Any conclusions made from this data exploration will be most accurate when applied to thinking about "average" wines as an overwhelming amount of wine data comes from wines scoring a 5 or a 6. 
